...
# 复用项目里「中文路径安全」的图片读取：cv2.imread 在含中文的路径下会返回 None，
# 而本项目就放在中文目录里。
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from src.utils.common import imread_unicode  # noqa: E402
...
def load_image(image_path: Path) -> cv2.Mat:
    ...
    image = imread_unicode(str(image_path), cv2.IMREAD_COLOR)
    if image is None:
